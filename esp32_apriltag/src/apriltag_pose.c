...
double orthogonal_iteration(matd_t** v, matd_t** p, matd_t** t, matd_t** R, int n_points, int n_steps) {
    ...
    for (int i = 0; i < n_steps; i++) {
        ...
        *R = matd_op("M*M'", M3_svd.U, M3_svd.V);
        // RESTORED FROM UPSTREAM (AprilRobotics/apriltag). The esp-idf fork
        // this port is vendored from predates this guard and omits it. Without
        // it, U*V' can come back with det = -1, i.e. a REFLECTION rather than
        // a rotation, whenever the SVD orders the singular vectors that way.
        // A reflected "pose" then flows into the upright test and the world
        // transform as if it were valid.
