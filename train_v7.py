#!/usr/bin/env python3
"""Training loop v7: stronger placement nudge + warm-start from v6.

Key changes from v6:
  - PLACEMENT_SCALE increased from 0.1 to 0.5 (middle ground between
    v5's too-gentle 0.1 and v4's too-aggressive 2.0)
  - Warm-starts from v6 weights (keeps excellent random grasping)
  - Keeps v6 replay buffer (reward change is small)
  - Full random spawn from start (no curriculum — v6 already mastered it)
  - File logging + nohup-friendly
"""
...
PLACEMENT_SCALE = 0.5  # stronger nudge than v6 (0.1), gentler than v4 (2.0)

# No curriculum — full random from start (v6 already mastered grasping)
CURRICULUM_START_EP = 0
CURRICULUM_END_EP = 0        # immediately full random
...
def placement_nudge(obs, next_obs, goal, gamma=0.99):
    ...
    change the optimal policy. Scale = 0.5 (stronger than v6's 0.1).
    """
    prev_bread = obs[0:3]
    bread = next_obs[0:3]
...
def train(env_name="PickPlace", n_envs=28, n_episodes=60000):
    print("=" * 70)
    print(f"TRAINING v7: {env_name} (stronger placement nudge)")
    print("=" * 70)
    ...
    # --- File logger ---
    log_file = os.path.join('.', 'logs', f'{env_name}_v7', 'training.log')
    ...
    # --- TensorBoard ---
    tb_dir = os.path.join(".", "logs", f"{env_name}_v7")
    ...
    print(f"  Curriculum:  NONE (full random from start)")
...
if __name__ == "__main__":
    ...
    # --- Warm-start from v6 ---
    WARMSTART_FROM = './checkpoints/td3_v6'

    print(f"\n{'=' * 70}")
    print("TRAINING v7 (stronger placement nudge + warm-start from v6)")
    print(f"{'=' * 70}")
    ...
    # --- Warm-start logic ---
    import shutil
    v7_dir = './checkpoints/td3_v7'
    if WARMSTART_FROM and not os.path.isdir(v7_dir):
        print(f"Copying v6 checkpoint to v7 dir...")
        shutil.copytree(WARMSTART_FROM, v7_dir)
        print(f"✓ Copied {WARMSTART_FROM} → {v7_dir}")
        print(f"  (v7 will resume from these weights + buffer)\n")
