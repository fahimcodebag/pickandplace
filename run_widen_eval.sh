#!/usr/bin/env bash
# Evaluate wide128 (Net2Wider 128/64) against cont64 (same parent, same 6000
...
O=Results/widen; mkdir -p $O/eval
...
for sd in 0 1 2 3 4 5; do conv wide128 128 64 $sd; conv cont64 64 32 $sd; done
: > $O/eval/jobs.txt
for sd in 0 1 2 3 4 5; do
  for arm in wide128 cont64; do
    [ "$arm" = wide128 ] && W="--grasp-fc1 128 --grasp-fc2 64" || W=""
