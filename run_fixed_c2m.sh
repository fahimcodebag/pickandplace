#!/usr/bin/env bash
# Re-measure the FIXED-SPAWN cells on the validated artifact (c2m512_s1).
...
O=Results/fixed_c2m512; mkdir -p $O
SEEDS="7 31 47 89 101 123 211 307 401 503 555 2024"
: > $O/jobs.txt
for s in $SEEDS; do
  echo "$PY fsm_sim.py --fixed-spawn --grasp-ckpt $C --place-ckpt $P --regrasp --rc-steps 60 --episodes 100 --seed $s --tag c2m512_fixed_fp32 --out $O/fp32_e$s.csv > $O/fp32_e$s.log 2>&1" >> $O/jobs.txt
  echo "$TFPY fsm_sim.py --fixed-spawn --int8 --grasp-tflite $GT --place-tflite $PT --grasp-ckpt $C --place-ckpt $P --regrasp --rc-steps 60 --episodes 100 --seed $s --tag c2m512_fixed_int8 --out $O/int8_e$s.csv > $O/int8_e$s.log 2>&1" >> $O/jobs.txt
done
